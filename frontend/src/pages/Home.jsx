...
function Home() {
	...
	useEffect(() => {
		...
		axios.get(`https://jos6ylumd75az7s4a5ajqyaqoi0iafmd.lambda-url.us-west-2.on.aws/reccomendations/${docSnap.id}`)
			.then((response) => {
				setUserList(response.data);
				setLoadingRecUsers(false);
			});
		axios.get("https://4ltkqflxgpkhdmqkrjm5w3ia340gceyn.lambda-url.us-west-1.on.aws/allUsers")
			.then((response => {
				response.data.forEach((user) => {
					user.portfolio.forEach((curr_portfolio) => {
						if (user.id != docSnap.id) {
							setSkillCounts((prevState) => ({
								...prevState,
								[curr_portfolio.skill]: prevState[curr_portfolio.skill] + 1
							}));
						}
					});
				});
